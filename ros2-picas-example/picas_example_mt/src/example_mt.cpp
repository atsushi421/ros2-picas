...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<trace::Trace> trace_latency = std::make_shared<trace::Trace>("data/R.txt");
    ...
    exec1.rt_attr.sched_priority = 90;
...
/// Partitioned scheduling (Using Thread affinity API)
    // int C1_thread_affinity= 0;
    // int C2_thread_affinity = 1;
    // int C3_thread_affinity = 2;
    // int C4_thread_affinity = 3;


    // exec1.set_thread_affinity(task1->timer_, C1_thread_affinity);
    // exec1.set_thread_affinity(task2->subscription_, C1_thread_affinity);

    // exec1.set_thread_affinity(task3->subscription_, C2_thread_affinity);
    // exec1.set_thread_affinity(task4->subscription_, C2_thread_affinity);
    // exec1.set_thread_affinity(task5->subscription_, C2_thread_affinity);

    // exec1.set_thread_affinity(task6->timer_, C3_thread_affinity);
    // exec1.set_thread_affinity(task7->subscription_, C3_thread_affinity);
    // exec1.set_thread_affinity(task8->subscription_, C3_thread_affinity);

    // exec1.set_thread_affinity(task9->timer_, C4_thread_affinity);
    // exec1.set_thread_affinity(task10->subscription_, C4_thread_affinity);
    // exec1.set_thread_affinity(task11->subscription_, C4_thread_affinity);
    // exec1.set_thread_affinity(task12->subscription_, C1_thread_affinity);   // added beside C1 so all threads have 4 callbacks

/// semi-Partitioned scheduling (Using Thread affinity API)

    // exec1.set_thread_affinity(task1->timer_, 3);
    // exec1.set_thread_affinity(task6->timer_, 2);
    // exec1.set_thread_affinity(task9->timer_, 1);

    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task1->get_name(), task1->timer_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task2->get_name(), task2->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task3->get_name(), task3->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task4->get_name(), task4->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task5->get_name(), task5->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task6->get_name(), task6->timer_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task7->get_name(), task7->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task8->get_name(), task8->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task9->get_name(), task9->timer_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task10->get_name(), task10->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task11->get_name(), task11->subscription_->thread_affinity);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "callback %s on thread# %d", task12->get_name(), task12->subscription_->thread_affinity);
